 ...
 *
 *   Tutorial 01 — DataStore, DatabaseConnection, DatabaseFunctions
 *   Tutorial 02 — UIFill (JTable, JComboBox, JList from ResultSet)
 *   Tutorial 03 — AppEventBus, EventWatcher, MessageLog
 *   Tutorial 04 — Loader, AppConfig, SplashScreen, SwingWorker
 *   Tutorial 05 — CustomerModel, ProductModel, BillModel, AbstractPerson
 *
 * Boot sequence (Tutorial 04 pattern):
 *   1. Show SplashScreen
 *   2. Run Loader on background thread
 *   3. Loader initializes: MessageLog → AppEventBus → AppConfig → DataStore
 *   4. If OK → show MainWindow
 *   5. If FAIL → show error dialog (DB not connected = warning only, app still runs)
 */
public class Main {
