...
/**
 * SMF Shop - CustomersPanel
 *
 * Fixes: try-with-resources on all CloseableResult usage.
 */
public class CustomersPanel extends JPanel implements EventWatcher {
    ...
    public void refresh() {
        try (DataStore.CloseableResult cr = CustomerModel.findAll()) {
            ...
            smf.messages.MessageLog.getInstance().error("[CustomersPanel] refresh: " + e.getMessage());
        }
    }
    ...
    private static JTextField field(String t) { return new JTextField(t, 8); }
}
